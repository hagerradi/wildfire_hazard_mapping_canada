...
from data_preparation.grid_loader.utils import MAX_WIND_VELOCITY, NODATA, load_raster


def convert_wind_angle_speed_to_uv(
    speed: np.ndarray, wd_deg: np.ndarray, nodata: float = -9999.0
...
def get_all_wind_grids(
    all_wind_velocity_files: list[Path], max_wind_velocity: float
    ...
    for wind_velocity_filename in all_wind_velocity_files:
        ...
        wind_velocity_grid = load_raster(str(wind_velocity_filename))
        wind_angle_grid = load_raster(str(wind_ang_filename))
...
def sample_wind_grids(all_u: list, all_v: list, all_mask: list, sampling: str = "all") -> np.ndarray:
    """
...
def load_wind_grid(path: str, sampling: str = "all") -> np.ndarray:
    ...
    """
    all_wind_velocity_files = list(Path(path).glob("w???_vel.asc"))
