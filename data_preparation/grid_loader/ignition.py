...
def build_esc_fire_distribution_zone_mapping(
    esc_fire_distribution_file_path: str, cause: int = None, season: int = None
) -> dict[int, float]:
    ...
    if cause is not None and season is not None:
        ...
        # aggregate over all
        esc_fire_dist_subset = esc_fire_dist.groupby("zone", as_index=False)["esc_fires"].max()
...
def load_fire_density_grid(
    zone_grid_file_path: str, esc_fire_distribution_file_path: str, season: int = None, cause: int = None
) -> np.ndarray:
    ...
    esc_fire_zone_prob_mapping = build_esc_fire_distribution_zone_mapping(
        esc_fire_distribution_file_path=esc_fire_distribution_file_path, cause=cause, season=season
    )
...
def load_ignition_grid(ignition_grids_folder_path: str, cause: int = None, season: int = None) -> np.ndarray:
    ...
    if season and cause:
        ...
        ignition_raster = ignition_raster.filled(NODATA)  # type: ignore
    ...
    # size (H, W)
    out_ignition_grids = np.stack(out_ignition_grids, axis=0)  # type: ignore
...
if __name__ == "__main__":
    ...
    out_ignition_grid = load_ignition_grid(
        ignition_grids_folder_path=os.path.join(root_dir, "ignitions_module/ignition_grids"), season=season, cause=cause
    )
