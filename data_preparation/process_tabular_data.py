...
def main():
    ...
    weather_norm_params_path = save_dir / args.weather_norm_params_file if args.weather_norm_params_file else None
    fire_size_norm_params_path = save_dir / args.fire_size_norm_params_file if args.fire_size_norm_params_file else None

    # If all norm params files already exist on disk, we are in inference / test-only
    # mode and there is nothing to fit — no train split is needed.
    all_norm_params_exist = (weather_norm_params_path is not None and weather_norm_params_path.exists()) and (
        fire_size_norm_params_path is not None and fire_size_norm_params_path.exists()
    )
    train_split_path = None if all_norm_params_exist else save_dir / args.train_split_file if args.train_split_file else None
