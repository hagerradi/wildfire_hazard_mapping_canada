...
def _train_hex_ids(train_split_path: Path | None) -> set[str] | None:
    ...
    if not train_split_path.exists():
        logger.warning("Training split %s not found; falling back to full-table tabular preprocessing stats.", train_split_path)
        return None
...
def _train_firezone_ids(
    *,
    ...
    if train_split_path is None:
        return None
    if not train_split_path.exists():
        logger.warning("Training split %s not found; falling back to full-table fire-size normalization stats.", train_split_path)
        return None
...
def build_weather_table(
    ...
    # Apply weather-specific normalization, fitting scalers on train hexes when a train split is available.
    df_weather = preprocess_weather_list(df_weather.drop(columns=["__hex_id"]), fit_mask=fit_mask)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Training split CSV in save_dir used to fit tabular preprocessing stats. If missing, falls back to full-table stats.",
    )
    ...
    train_split_path = save_dir / args.train_split_file if args.train_split_file else None
    ...
    build_weather_table(root_dir=root_dir, save_path=weather_save_path, train_split_path=train_split_path)
    train_firezone_ids = _train_firezone_ids(
        data_dir=save_dir,
