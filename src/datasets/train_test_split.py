import os
from pathlib import Path

import pandas as pd


def get_hex_ids(folder_path:str)->list:
    """
    Finds all hex_ids from the metadata df files
    """
    folder = Path(folder_path)
    hex_ids = []
    for file_path in folder.glob("meta_hex_*.csv"):
        filename_no_ext = file_path.stem 
        extracted_id = filename_no_ext.removeprefix("meta_hex_")
        
        hex_ids.append(extracted_id)
        
    return hex_ids

def get_selected_hexel_metadata(data_dir:str, hex_ids:list)->pd.DataFrame:
    """
        Combine all the selected dfs for the train/val/eval sets
    """
    df_list = [pd.read_csv(os.path.join(data_dir, f"meta_hex_{hex_id}.csv")) for hex_id in hex_ids]
    df =  pd.concat(df_list, ignore_index=True)
    return df

def get_train_test_split(data_dir:str, val_hex_id:list, eval_hex_id:list):
    """Split the data into train, val and test sets using the metadata csvs created"""
    #Get the train ids
    all_hex_ids = get_hex_ids(data_dir)
    train_hex_ids = list(set(all_hex_ids) -  set(val_hex_id)- set(eval_hex_id))

    train_df = get_selected_hexel_metadata(data_dir, hex_ids=train_hex_ids)
    val_df = get_selected_hexel_metadata(data_dir, hex_ids=val_hex_id)
    eval_df = get_selected_hexel_metadata(data_dir, hex_ids=eval_hex_id)

    print("Total number of samples in train, val, eval are:",len(train_df), len(val_df), len(eval_df))
    
    train_df.to_csv(os.path.join(data_dir, "train.csv"), index=False)
    val_df.to_csv(os.path.join(data_dir, "val.csv"), index=False)
    eval_df.to_csv(os.path.join(data_dir, "eval.csv"), index=False)

if __name__ == "__main__":
    data_dir = "../yan_bp3/data_samples_approach_2"
    get_train_test_split(data_dir=data_dir, val_hex_id=["16"], eval_hex_id=["41"])