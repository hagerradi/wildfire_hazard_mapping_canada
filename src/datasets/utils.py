...
def get_dataset_dimensions(dataset) -> tuple[int | None, dict[str, int]]:
    ...
    for name, source in sources.items():
        if name == "grid":
            ...
            iros_len = getattr(source, "iros_vector_len", 0)
            if iros_len > 0:
                auxiliary_input_dims["iros"] = iros_len
        elif name in SPATIALIZED_TABULAR_SOURCE_NAMES:
...
def get_iros_normalization_stats(dataset) -> tuple[torch.Tensor | None, torch.Tensor | None]:
    """
    Returns ``(iros_mean, iros_std)`` tensors from the GridSource if iROS encoding is active,
    otherwise returns ``(None, None)``.

    These stats are computed from training hexels only inside ``GridSource.__init__`` and
    should be passed to the model at construction time so that ``iROSEncoder`` normalizes
    ...
    if grid_source is None or getattr(grid_source, "iros_vector_len", 0) == 0:
        return None, None
    mean = getattr(grid_source, "iros_mean", None)
    std = getattr(grid_source, "iros_std", None)
