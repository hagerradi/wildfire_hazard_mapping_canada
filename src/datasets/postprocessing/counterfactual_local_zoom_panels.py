...
def select_component_windows(
    ...
    *,
    ...
    selected = sorted(candidates, key=lambda item: int(item["component_pixels"]), reverse=True)[:n_components]
    ...
    for rank, item in enumerate(selected, start=1):
        ...
        windows.append(
            ComponentWindow(
                ...
                rank=rank,
                component_pixels=int(item["component_pixels"]),
...
def crop(data: np.ndarray | np.ma.MaskedArray, window: ComponentWindow) -> np.ma.MaskedArray:
...
def _norm_limits(norm: Normalize) -> str:
    return f"[{float(norm.vmin):.2g}, {float(norm.vmax):.2g}]"
...
def _mean_on_crop(data: np.ndarray | np.ma.MaskedArray, window: ComponentWindow) -> float:
