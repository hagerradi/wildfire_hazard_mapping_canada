...
def _load_raw_weather_with_hex_id(path: Path) -> pd.DataFrame:
    ...
    frame.insert(0, "__hex_id", _hex_id_from_weather_path(path))
    return frame
...
def apply_external_mean_zone_transplant(
    ...
    *,
    ...
    """Give recipient zones the exact mean processed-weather vector of donor hexels.

    The returned table contains one row per WeatherZone. Baseline zone means are
    retained everywhere except recipient zones, which receive the donor mean exactly
    rather than a diluted mean over all hexels sharing those zones.
    """
    ...
    if "WeatherZone" not in processed.columns:
        raise ValueError("Processed weather table is missing column 'WeatherZone'.")
    ...
    donor_mask = raw_features["__hex_id"].isin(normalized_donor_ids).to_numpy()
    ...
    for recipient_hex_id in normalized_recipient_ids:
        ...
        reports.append(
            WeatherEditReport(
                ...
                n_recipient_zones=int(processed.loc[recipient_mask, "WeatherZone"].nunique()),
