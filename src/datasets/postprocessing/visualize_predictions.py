...
def _as_float_array_with_nan(grid: np.ndarray) -> np.ndarray:
...
def _valid_pair_values(gt_grid: np.ndarray, pred_grid: np.ndarray, hex_id: str | int) -> tuple[np.ndarray, np.ndarray]:
    gt_arr = _as_float_array_with_nan(gt_grid)
    pred_arr = _as_float_array_with_nan(pred_grid)
...
def _value_scale_values(gt_grid: np.ndarray, pred_grid: np.ndarray, source: str) -> np.ndarray:
    if source == "paired":
        values = np.concatenate([gt_grid[np.isfinite(gt_grid)], pred_grid[np.isfinite(pred_grid)]])
    elif source == "target":
        values = gt_grid[np.isfinite(gt_grid)]
    elif source == "prediction":
        values = pred_grid[np.isfinite(pred_grid)]
    else:
        raise ValueError(f"value_percentile_source must be one of ('paired', 'target', 'prediction'), got {source!r}.")
    if values.size == 0:
        raise ValueError(f"No finite values available for value scale source {source!r}.")
    return values
...
def visualize_target_grids(
    ...
    value_percentile_source: str = "paired",
    ...
    if actual_support_mask is not None:
        actual_support_mask = np.asarray(actual_support_mask, dtype=bool)
        if actual_support_mask.shape != pred_mask.shape:
            raise ValueError(
                f"actual_support_mask shape {actual_support_mask.shape} does not match plot shape {pred_mask.shape} for hex {hex_id}."
            )
    if buffer_support_mask is not None:
        buffer_support_mask = np.asarray(buffer_support_mask, dtype=bool)
        if buffer_support_mask.shape != pred_mask.shape:
            raise ValueError(
                f"buffer_support_mask shape {buffer_support_mask.shape} does not match plot shape {pred_mask.shape} for hex {hex_id}."
            )
    # Shared scale for GT and Prediction
    scale_values = _value_scale_values(gt_grid=gt_grid, pred_grid=pred_grid, source=value_percentile_source)
    paired_values = _value_scale_values(gt_grid=gt_grid, pred_grid=pred_grid, source="paired")
    shared_vmin = float(np.nanmin(paired_values))
    shared_vmax = _percentile_or_extreme(scale_values, value_percentile)
...
def visualize_hexel_iou(
    ...
    gt_grid = _as_float_array_with_nan(gt_grid)
    pred_grid = _as_float_array_with_nan(pred_grid)
    ...
    if actual_support_mask is not None:
        actual_support_mask = np.asarray(actual_support_mask, dtype=bool)
        if actual_support_mask.shape != valid_mask.shape:
            raise ValueError(
                f"actual_support_mask shape {actual_support_mask.shape} does not match IoU plot shape {valid_mask.shape} for hex {hex_id}."
            )
    if buffer_support_mask is not None:
        buffer_support_mask = np.asarray(buffer_support_mask, dtype=bool)
        if buffer_support_mask.shape != valid_mask.shape:
            raise ValueError(
                f"buffer_support_mask shape {buffer_support_mask.shape} does not match IoU plot shape {valid_mask.shape} for hex {hex_id}."
            )
...
def get_distribution_axis_limit(gt_vals: np.ndarray, pred_vals: np.ndarray, probability_scale: bool) -> float:
    ...
    gt_vals = _as_float_array_with_nan(gt_vals)
    pred_vals = _as_float_array_with_nan(pred_vals)
