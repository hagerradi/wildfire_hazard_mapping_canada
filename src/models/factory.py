...
SMP_NAMES = {"smp", "segmentation_models_pytorch", "smp_dense"}
    ...
    *,
    ...
    if architecture in MULTI_SOURCE_UNET_NAMES:
        ...
        return MultiSourceUNet(
            ...
            use_coordconv=model_config.use_coordconv,
            use_multiscale_global_context=model_config.use_multiscale_global_context,
        )

    if architecture in SMP_NAMES:
        if auxiliary_requested and not auxiliary_input_dims:
            raise ValueError("Config requests auxiliary features, but no auxiliary dimensions were detected.")
        return SMPDensePredictionModel(
            architecture=model_config.smp_architecture,
            encoder_name=model_config.smp_encoder_name,
            encoder_weights=model_config.smp_encoder_weights,
            input_channels=spatial_input_channels,
            num_classes=model_config.num_classes,
            use_coordconv=model_config.use_coordconv,
            auxiliary_input_dims=auxiliary_input_dims if auxiliary_requested else {},
            auxiliary_feature_encoder_poolings=model_config.auxiliary_feature_encoder_poolings,
            smp_kwargs=model_config.smp_kwargs,
        )

    supported = sorted(BASELINE_UNET_NAMES | MULTI_SOURCE_UNET_NAMES | SMP_NAMES | {"auto"})
