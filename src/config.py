...
class TabularParams(BaseModel):
    ...
    zone_selection_approach: str = "mode"  # Selection method to determine weather zone to be used for the patch: "mode" (for most common zone) or "weighted" (for frequency-weighted sampling)
    num_samples_per_patch: int = 256
    transforms_list: list[str] = Field(default_factory=list)
    augmentation_prob: float = 0.0
    sampling_bias: str | None = (
        None  # Whether to bias sampling towards high or low values of the feature of interest, or no bias (None, "high", "low")
    )
